...
export default function AccountsPayable() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await api.delete(`/accounts-payable/${accountToDelete.id}`);
      fetchAccounts();
      fetchStats();
            ...
            <div className="fixed inset-0 flex items-center justify-center z-[120] p-4 pointer-events-none">
              ...
              >
                <div className="p-6">
                  ...
                  <h3 className="text-lg font-bold text-slate-900 mb-2 italic tracking-tighter uppercase">Excluir Conta</h3>
                  <p className="text-slate-600 text-sm mb-6 font-medium">
                    Tem certeza que deseja excluir esta conta de <span className="font-black text-slate-900">{accountToDelete?.supplier_name || 'Despesa Geral'}</span>? Esta ação não pode ser desfeita e afetará o fluxo de caixa histórico.
                  </p>
                  <div className="flex gap-3">
                    <button
                      onClick={() => setDeleteConfirmOpen(false)}
